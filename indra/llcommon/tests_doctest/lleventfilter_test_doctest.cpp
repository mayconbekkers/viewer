// ---------------------------------------------------------------------------
// Auto-generated from lleventfilter_test.cpp at 2025-10-16T20:37:40+00:00
...
TUT_SUITE("lleventfilter")
{
    TUT_CASE("lleventfilter_test::LLEventMatching")
    {
        using namespace tut;
        TUT_SET_TEST_NAME("LLEventMatching");
    ...
    TUT_CASE("lleventfilter_test::LLEventTimeout::actionAfter()")
    {
        using namespace tut;
        TUT_SET_TEST_NAME("LLEventTimeout::actionAfter()");
        LLEventPump& driver(pumps.obtain("driver"));
    ...
    TUT_CASE("lleventfilter_test::LLEventTimeout::eventAfter()")
    {
        using namespace tut;
        TUT_SET_TEST_NAME("LLEventTimeout::eventAfter()");
        LLEventPump& driver(pumps.obtain("driver"));
        TestEventTimeout filter(driver);
        listener0.reset(0);
        LLTempBoundListener temp1(
    ...
    TUT_CASE("lleventfilter_test::LLEventTimeout::errorAfter()")
    {
        using namespace tut;
        TUT_SET_TEST_NAME("LLEventTimeout::errorAfter()");
        WrapLLErrs capture;
        LLEventPump& driver(pumps.obtain("driver"));
        TestEventTimeout filter(driver);
        listener0.reset(0);
        ...
        TUT_ENSURE_CONTAINS("errorAfter() timeout exception", threw, "timeout");
        // Timing out cancels the timer. Verify that.
        listener0.reset(0);
        filter.forceTimeout();
        mainloop.post(17);
    ...
    TUT_CASE("lleventfilter_test::LLEventThrottle")
    {
        using namespace tut;
        TUT_SET_TEST_NAME("LLEventThrottle");
        ...
        throttle.advance(1);
        throttle.post("1");
        TUT_ENSURE_EQ("1", cat.result, "1"); // delivered immediately
        //  2: post(): deferred: waiting for 3 seconds to elapse
        throttle.advance(1);
        throttle.post("2");
        TUT_ENSURE_EQ("2", cat.result, "1"); // "2" not yet delivered
        //  3: post(): deferred
        throttle.advance(1);
        throttle.post("3");
        TUT_ENSURE_EQ("3", cat.result, "1"); // "3" not yet delivered
        //  4: no post() call, but event delivered to listeners; next no sooner than 7
        throttle.advance(1);
        TUT_ENSURE_EQ("4", cat.result, "13"); // "3" delivered
        //  6: post(): deferred
        throttle.advance(2);
        throttle.post("6");
        TUT_ENSURE_EQ("6", cat.result, "13"); // "6" not yet delivered
        //  7: no post() call, but event delivered; next no sooner than 10
        throttle.advance(1);
        TUT_ENSURE_EQ("7", cat.result, "136"); // "6" delivered
        // 12: post(): immediately passed to listeners, next no sooner than 15
        throttle.advance(5);
        throttle.post(";12");
        TUT_ENSURE_EQ("12", cat.result, "136;12"); // "12" delivered
        // 17: post(): immediately passed to listeners, next no sooner than 20
        throttle.advance(5);
        throttle.post(";17");
        TUT_ENSURE_EQ("17", cat.result, "136;12;17"); // "17" delivered
    }

    TUT_CASE("lleventfilter_test::LLEventMailDrop")
    {
        using namespace tut;
    ...
    TUT_CASE("lleventfilter_test::LLEventLogProxyFor<LLEventMailDrop>")
    {
        using namespace tut;
