...
namespace llmath_doctest
{
constexpr F32 kApproxTolerance32 = 1.0e-5f;
constexpr F64 kApproxTolerance64 = 1.0e-8;
constexpr F32 kApproxRelativeTolerance32 = 5.0e-7f;
constexpr F64 kApproxRelativeTolerance64 = 5.0e-13;

inline bool check_approx(F32 actual, F32 expected, F32 tolerance = kApproxTolerance32)
{
    const F32 scale = std::max(std::fabs(actual), std::fabs(expected));
    const F32 relative = scale * kApproxRelativeTolerance32;
    const F32 final_tolerance = std::max(tolerance, relative);
    LL_CHECK_NEAR(actual, expected, final_tolerance);
    return true;
}

inline bool check_approx(F64 actual, F64 expected, F64 tolerance = kApproxTolerance64)
{
    const F64 scale = std::max(std::fabs(actual), std::fabs(expected));
    const F64 relative = scale * kApproxRelativeTolerance64;
    const F64 final_tolerance = std::max(tolerance, relative);
    LL_CHECK_NEAR(actual, expected, final_tolerance);
    return true;
}

inline bool check_fraction(F32 actual, F32 expected, U32 frac_bits)
{
    const F32 diff = std::fabs(actual - expected);
    const S32 diff_integer = static_cast<S32>(diff);
    const F32 fractional = diff - static_cast<F32>(diff_integer);
    const S32 diff_fraction = static_cast<S32>(fractional * static_cast<F32>(1u << frac_bits));
    INFO("fraction diff=" << diff << " bits=" << frac_bits);
    LL_CHECK_MSG(diff_integer == 0, "integer component differs beyond allowed fractional tolerance");
    LL_CHECK_MSG(diff_fraction <= 1, "fractional component differs beyond allowed fractional tolerance");
    return true;
}

inline bool check_fraction(F64 actual, F64 expected, U32 frac_bits)
{
    const F64 diff = std::fabs(actual - expected);
    const S32 diff_integer = static_cast<S32>(diff);
    const F64 fractional = diff - static_cast<F64>(diff_integer);
    const S32 diff_fraction = static_cast<S32>(fractional * static_cast<F64>(1ull << frac_bits));
    INFO("fraction diff=" << diff << " bits=" << frac_bits);
    LL_CHECK_MSG(diff_integer == 0, "integer component differs beyond allowed fractional tolerance");
    LL_CHECK_MSG(diff_fraction <= 1, "fractional component differs beyond allowed fractional tolerance");
    return true;
...
#undef is_approx_equal_fraction
#define is_approx_equal(expected, actual) ::llmath_doctest::check_approx((actual), (expected))
#define is_approx_equal_fraction(expected, actual, bits) \
    ::llmath_doctest::check_fraction((actual), (expected), (bits))
...
TUT_SUITE("llquaternion_test")
{
    ...
    {
        using namespace tut;
        const LLVector4 vect(12.0f, 5.0f, 60.0f, 75.1f);
        const LLQuaternion quat(2323.034f, 23.5f, 673.23f, 57667.5f);
        LLVector4 result = vect * quat;

        INFO("LLVector4 operator*(const LLVector4&, const LLQuaternion&) first case");
        ...
        LL_CHECK_NEAR(result.mV[3], 75.099998f, llmath_doctest::kApproxTolerance32);

        const LLVector4 vect1(22.0f, 45.0f, 40.0f, 78.1f);
        const LLQuaternion quat1(2.034f, 45.5f, 37.23f, 7.5f);
        result = vect1 * quat1;

        INFO("LLVector4 operator*(const LLVector4&, const LLQuaternion&) second case");
        is_approx_equal(-58153.5390f, result.mV[0]);
        LL_CHECK_NEAR(result.mV[1], 183787.8125f, llmath_doctest::kApproxTolerance32);
        LL_CHECK_NEAR(result.mV[2], 116864.164063f, llmath_doctest::kApproxTolerance32);
        LL_CHECK_NEAR(result.mV[3], 78.099998f, llmath_doctest::kApproxTolerance32);
    }
