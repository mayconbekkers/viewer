// ---------------------------------------------------------------------------
// Auto-generated from lltrace_test.cpp at 2025-10-16T20:37:41+00:00
...
TUT_SUITE("lltrace_test")
{
    TUT_CASE("lltrace_test::trace_object_t_test_1")
    {
        ...
        TUT_CHECK_MSG(at_work.getSum(sCupsOfCoffeeConsumed) == 3
                && all_day.getSum(sCupsOfCoffeeConsumed) == 5
                && after_3pm.getSum(sCupsOfCoffeeConsumed) == 2, "count stats are counted when recording is active");
        TUT_CHECK_MSG(at_work.getSum(sOuncesPerCup) == S32Ounces(48)
                && all_day.getSum(sOuncesPerCup) == S32Ounces(80)
                && after_3pm.getSum(sOuncesPerCup) == S32Ounces(36), "measurement sums are counted when recording is active");
        TUT_CHECK_MSG(at_work.getMin(sOuncesPerCup) == S32GrandeCup(1)
                && all_day.getMin(sOuncesPerCup) == S32TallCup(1)
                && after_3pm.getMin(sOuncesPerCup) == S32GrandeCup(1), "measurement min is specific to when recording is active");
        TUT_CHECK_MSG(at_work.getMax(sOuncesPerCup) == S32GrandeCup(1)
                && all_day.getMax(sOuncesPerCup) == S32VentiCup(1)
                && after_3pm.getMax(sOuncesPerCup) == S32VentiCup(1), "measurement max is specific to when recording is active");
        TUT_CHECK_MSG(at_work.getMin(sCaffeineLevelStat) == sCaffeinePerOz * ((S32Ounces)S32TallCup(1)).value()
                && all_day.getMin(sCaffeineLevelStat) == F32Milligrams(0.f)
                && after_3pm.getMin(sCaffeineLevelStat) == sCaffeinePerOz * ((S32Ounces)S32TallCup(1) + (S32Ounces)S32GrandeCup(2)).value(), "sample min is specific to when recording is active");
        TUT_CHECK_MSG(at_work.getMax(sCaffeineLevelStat) == sCaffeinePerOz * ((S32Ounces)S32TallCup(1) + (S32Ounces)S32GrandeCup(3)).value()
                ...
                && after_3pm.getMax(sCaffeineLevelStat) == sCaffeinePerOz * ((S32Ounces)S32TallCup(1) + (S32Ounces)S32GrandeCup(3) + (S32Ounces)S32VentiCup(1)).value(), "sample max is specific to when recording is active");
    }
}
