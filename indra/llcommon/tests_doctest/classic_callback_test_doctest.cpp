// ---------------------------------------------------------------------------
// Auto-generated from classic_callback_test.cpp at 2025-10-16T20:37:40+00:00
...
TUT_SUITE("classic_callback")
{
    TUT_CASE("classic_callback_test::ClassicCallback")
    {
        using namespace tut;
        TUT_SET_TEST_NAME("ClassicCallback");
        ...
        TUT_ENSURE_EQ("failed to call through ClassicCallback", mcb.mMsg, "called");

        // try with HeapClassicCallback
        mcb.mMsg.clear();
        auto hcbp{ makeHeapClassicCallback<callback_t>(mcb) };
        someAPI(hcbp->get_callback(), hcbp->get_userdata());
        TUT_ENSURE_EQ("failed to call through HeapClassicCallback", mcb.mMsg, "called");
        ...
        TUT_ENSURE_EQ("failed to call lambda", mcb.mMsg, "extra called");
        ...
        TUT_ENSURE_EQ("failed to return callback result", result, "hello back!");
        TUT_ENSURE_EQ("failed to set int", sData.mi, 17);
        TUT_ENSURE_EQ("failed to set string", sData.ms, "hello world");
        TUT_ENSURE_EQ("failed to set double", sData.mf, 3.0);
    }
}
