// ---------------------------------------------------------------------------
// Auto-generated from lldependencies_test.cpp at 2025-10-16T20:37:40+00:00
...
TUT_SUITE("LLDependencies")
{
    TUT_CASE("lldependencies_test::deps_object_test_1")
    {
        using namespace tut;
                ...
                deps.add("lazy");
                TUT_ENSURE_EQ(sorted_keys(deps), make<StringList>(list_of("lazy")));
                deps.add("jumps");
                TUT_CHECK_MSG(deps.get("lazy"), "found lazy");
                TUT_CHECK_MSG(! deps.get("dog."), "not found dog.");
                ...
                // test output.
                TUT_ENSURE_EQ(sorted_keys(deps), make<StringList>(list_of("lazy")("jumps")));
                deps.add("The", 0, empty, list_of("fox")("dog."));
                // Test key accessors
                TUT_CHECK_MSG(is_empty(deps.get_before_range("bogus")), "empty before deps for missing key");
                TUT_CHECK_MSG(is_empty(deps.get_before_range("jumps")), "empty before deps for jumps");
                TUT_ENSURE_EQ(instance_from_range< std::set<std::string> >(deps.get_before_range("The")), make< std::set<std::string> >(list_of("dog.")("fox")));
                // resume building dependencies
                TUT_ENSURE_EQ(sorted_keys(deps), make<StringList>(list_of("lazy")("jumps")("The")));
                deps.add("the", 0, list_of("The"));
                TUT_ENSURE_EQ(sorted_keys(deps), make<StringList>(list_of("lazy")("jumps")("The")("the")));
                deps.add("fox", 0, list_of("The"), list_of("jumps"));
                TUT_ENSURE_EQ(sorted_keys(deps), make<StringList>(list_of("lazy")("The")("the")("fox")("jumps")));
        ...
        |*==========================================================================*/
                ...
                TUT_ENSURE_EQ(instance_from_range< std::set<std::string> >(deps.get_after_range("dog.")), make< std::set<std::string> >(list_of("lazy")("yellow")));
                TUT_ENSURE_EQ(sorted_keys(deps), make<StringList>(list_of("lazy")("The")("the")("fox")("jumps")("dog.")));
                deps.add("quick", 0, list_of("The"), list_of("fox")("brown"));
                TUT_ENSURE_EQ(sorted_keys(deps), make<StringList>(list_of("lazy")("The")("the")("quick")("fox")("jumps")("dog.")));
                deps.add("over", 0, list_of("jumps"), list_of("yellow")("the"));
                TUT_ENSURE_EQ(sorted_keys(deps), make<StringList>(list_of("lazy")("The")("quick")("fox")("jumps")("over")("the")("dog.")));
                deps.add("yellow", 0, list_of("the"), list_of("lazy"));
                TUT_ENSURE_EQ(sorted_keys(deps), make<StringList>(list_of("The")("quick")("fox")("jumps")("over")("the")("yellow")("lazy")("dog.")));
                deps.add("brown");
                // By now the dependencies are pretty well in place. A change to THIS
                // order should be viewed with suspicion.
                TUT_ENSURE_EQ(sorted_keys(deps), make<StringList>(list_of("The")("quick")("brown")("fox")("jumps")("over")("the")("yellow")("lazy")("dog.")));

                StringList keys(make<StringList>(list_of("The")("brown")("dog.")("fox")("jumps")("lazy")("over")("quick")("the")("yellow")));
                TUT_ENSURE_EQ(instance_from_range<StringList>(deps.get_key_range()), keys);
        #if (! defined(__GNUC__)) || (__GNUC__ > 3) || (__GNUC__ == 3 && __GNUC_MINOR__ > 3)
                // This is the succinct way, works on modern compilers
                TUT_ENSURE_EQ(instance_from_range<StringList>(make_transform_range(deps.get_range(), extract_key)), keys);
        #else   // gcc 3.3
                ...
                {
                    TUT_ENSURE_EQ(kni->first, *ki);
                }
                TUT_CHECK_MSG(kni == knend && ki == kend, "get_range() returns proper length");
        #endif  // gcc 3.3
    ...
    TUT_CASE("lldependencies_test::deps_object_test_2")
    {
        using namespace tut;
                ...
                TUT_ENSURE_EQ(range.begin()->first, "abc");
                TUT_ENSURE_EQ(range.begin()->second, 1);
                ...
                ++const_iterator;
                TUT_ENSURE_EQ(const_iterator->first, "def");
                TUT_ENSURE_EQ(const_iterator->second, 2);
        ...
        //        *node_range.begin() = 1;
                ...
                TUT_ENSURE_EQ(instance_from_range<std::vector<int> >(const_node_range), make< std::vector<int> >(list_of(1)(2)(3)));
                NameIndexDeps::const_key_range const_key_range(const_nideps.get_key_range());
                TUT_ENSURE_EQ(instance_from_range<StringList>(const_key_range), make<StringList>(list_of("abc")("def")("ghi")));
                NameIndexDeps::sorted_range sorted(const_nideps.sort());
                NameIndexDeps::sorted_iterator sortiter(sorted.begin());
                TUT_ENSURE_EQ(sortiter->first, "ghi");
                TUT_ENSURE_EQ(sortiter->second, 3);

                // test all iterator-flavored versions of get_after_range()
                StringList def(make<StringList>(list_of("def")));
                TUT_CHECK_MSG(is_empty(nideps.get_before_range(nideps.get_range().begin())), "empty abc before list");
                TUT_ENSURE_EQ(instance_from_range<StringList>(nideps.get_after_range(nideps.get_range().begin())), def);
                TUT_ENSURE_EQ(instance_from_range<StringList>(const_nideps.get_after_range(const_nideps.get_range().begin())), def);
        //        ensure_equals(instance_from_range<StringList>(nideps.get_after_range(nideps.get_node_range().begin())),
        //                      def);
                TUT_ENSURE_EQ(instance_from_range<StringList>(const_nideps.get_after_range(const_nideps.get_node_range().begin())), def);
                TUT_ENSURE_EQ(instance_from_range<StringList>(nideps.get_after_range(nideps.get_key_range().begin())), def);
                // advance from "ghi" to "def", which must come after "ghi"
                ++sortiter;
                TUT_ENSURE_EQ(instance_from_range<StringList>(const_nideps.get_after_range(sortiter)), make<StringList>(list_of("ghi")));
    }
}
