// ---------------------------------------------------------------------------
// Auto-generated from lltreeiterators_test.cpp at 2025-10-16T20:37:41+00:00
...
TUT_SUITE("LLTreeIterators")
{
    TUT_CASE("lltreeiterators_test::iter_object_test_1")
    {
        ...
        //      std::cout << desc1 << ":\n";
                ...
                {
        //          std::cout << (*pni)->name() << '\n';
                    TUT_CHECK_MSG(false, "LLLinkedIter<PlainNode>(NULL)");
                }
                TUT_CHECK_MSG(verify(desc1,
        ...
        //          std::cout << "Second is " << (*pni  )->name() << '\n';
                ...
                {
                    LLLinkedIter<PlainNode> pni(first, boost::bind(&PlainNode::next, _1));
                    TUT_ENSURE_EQ("first", (*pni++)->name(), "a");
                    TUT_ENSURE_EQ("second", (*pni  )->name(), "b");
                }
    }
    ...
    TUT_CASE("lltreeiterators_test::iter_object_test_2")
    {
        ...
        //      std::cout << "Now the LLLinkedIter<RCNode> is the only remaining reference to RCNode chain\n";
                TUT_ENSURE_EQ(last_RCNode_destroyed, "");
                TUT_ENSURE(rcni != end2);
                TUT_ENSURE_EQ((*rcni)->name(), "x");
                ++rcni;
                TUT_ENSURE_EQ(last_RCNode_destroyed, "x");
                TUT_ENSURE(rcni != end2);
                TUT_ENSURE_EQ((*rcni)->name(), "y");
                ++rcni;
                TUT_ENSURE_EQ(last_RCNode_destroyed, "y");
                TUT_ENSURE(rcni != end2);
                TUT_ENSURE_EQ((*rcni)->name(), "z");
                ++rcni;
                TUT_ENSURE_EQ(last_RCNode_destroyed, "z");
                TUT_ENSURE(rcni == end2);
    }
    ...
    TUT_CASE("lltreeiterators_test::iter_object_test_3")
    {
        ...
        //      set_test_name("LLTreeIter tests");
                TUT_CHECK_MSG(TreeNode::child_iterator>
                                       ("TreeNode",
                                        boost::bind(&TreeNode::getParent, _1),
                                        boost::bind(&TreeNode::child_begin, _1),
                                        boost::bind(&TreeNode::child_end, _1)), LLTreeIter_tests<TreeNode);
                TUT_CHECK_MSG(LLLinkedIter<PlainTree> >
                                       ("PlainTree",
                                        boost::bind(&PlainTree::mParent, _1),
                                        PlainTree_child_begin,
                                        PlainTree_child_end), LLTreeIter_tests<PlainTree);
    }
    ...
    TUT_CASE("lltreeiterators_test::iter_object_test_4")
    {
        ...
        //          std::cout << node->name() << '\n';
                ...
                TUT_CHECK_MSG(verify(desc1, getRootRange<LLTreeIter::UP>(tnB2b), RootExpected<LLTreeIter::UP>()), desc1);
        ...
        //          std::cout << node->name() << '\n';
                ...
                TUT_CHECK_MSG(verify(desc2, etnB2b->getRootRange<LLTreeIter::UP>(), RootExpected<LLTreeIter::UP>()), desc2);
    }
